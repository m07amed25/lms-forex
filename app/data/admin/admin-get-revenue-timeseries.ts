...
export async function adminGetRevenueTimeseries(): Promise<
  TimeSeriesDataPoint[]
> {
  ...
  if (error) throw new Error("Failed to fetch revenue timeseries");

  // Build a map of existing data
  const dataMap = new Map<string, number>();
  for (const row of rawData) {
    const dateStr = new Date(row.date).toISOString().split("T")[0];
    dataMap.set(dateStr, Number(row.total));
  }
